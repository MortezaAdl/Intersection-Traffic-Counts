...
class LoadImages:  # for inference
    def __init__(self, path, OIS_size, img_center, img_size=640, stride=32):
        ...
        self.mode = 'image'
        self.Square_size = min(OIS_size)
        self.LX = img_center - round(self.Square_size/2)
        self.RX = self.LX + self.Square_size
        rotation_angle = 0
        self.M = []
        for i in range(4):
            self.M.append(cv2.getRotationMatrix2D((self.Square_size/2, self.Square_size/2), rotation_angle, 1))
            rotation_angle += 90
    ...
    def __next__(self):
        ...
        self.image = img0[:, self.LX:self.RX]
        ...
        for i in range(4):
            # Rotate the image
            if i > 0:
                rotated_image = cv2.warpAffine(self.image, self.M[i], (self.Square_size, self.Square_size))
            else:
                rotated_image = self.image
            rotated_image = rotated_image[0:int(self.Square_size/2), :]
            Img0.append(rotated_image)
            # Padded resize
            img = letterbox(rotated_image, self.img_size, stride=self.stride)[0]
            # Convert
            img = img[:, :, ::-1].transpose(2, 0, 1)  # BGR to RGB, to 3x416x416
            img = np.ascontiguousarray(img)
            Img.append(img)
            
        return path, Img, Img0, self.cap, img0
